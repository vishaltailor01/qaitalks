...
export function identifyStrengths(
  ...
  // High proficiency skills (Expert/Advanced)
  ...
  // Intermediate skills as fallback or additional strengths
  const intermediateSkills = coreSkillsAssessment
    .filter((s) => s.proficiencyLevel === 'intermediate')
    .map((s) => s.skill);

  if (strengths.length < 2 && intermediateSkills.length > 0) {
    const count = Math.min(intermediateSkills.length, 2);
    strengths.push(`Proven experience in ${intermediateSkills.slice(0, count).join(' and ')}`);
  }

  // Tools proficiency
  const proficientTools = toolsAssessment
    .filter((s) => s.proficiencyLevel === 'advanced' || s.proficiencyLevel === 'expert' || s.found)
    .map((s) => s.skill);

  if (proficientTools.length > 0) {
    const toolCount = Math.min(proficientTools.length, 3);
    strengths.push(`Hands-on experience with ${proficientTools.slice(0, toolCount).join(', ')}`);
  }

  // QA-specific knowledge
  if (qaKeywordsFound.length >= 3) {
    strengths.push(`Solid QA domain knowledge (${qaKeywordsFound.length} key concepts identified)`);
  }

  return strengths.slice(0, 4); // Limit to top 4 strengths for UI clarity
}
